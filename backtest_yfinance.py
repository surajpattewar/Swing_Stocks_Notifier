...
def parse_args():
    ...
    parser.add_argument("--top-n-per-day", type=int, default=1,
                        help="Limit to top N signals per day (default: 1)")
...
def process_symbol(symbol: str, df: pd.DataFrame, beta: float, backtest_start: date, 
                   backtest_end: date, min_score: int, max_holding_days: int, index_df: pd.DataFrame = None) -> list[dict]:
    ...
    for eval_date in eval_dates:
        ...
        # Take the last 200 rows to ensure weekly EMA/RSI can be calculated
        df_slice_warm = df_slice.iloc[-200:] if len(df_slice) >= 200 else df_slice
        
        try:
            cand = evaluate(symbol, df_slice_warm, {"beta": beta}, index_df=index_df)
        except Exception:
            continue
...
def main():
    ...
    # Warmup buffer (200 trading days is ~300 calendar days)
    ...
    for idx_sym in ["^NSEI"]:
        try:
            logger.info(f"Pre-fetching benchmark {idx_sym} for backtesting...")
            ...
            logger.error(f"Failed to pre-fetch {idx_sym}: {e}")
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for sym, df in stock_data.items():
            ...
            futures[executor.submit(process_symbol, sym, df, beta, start_date, end_date, args.min_score, args.max_holding_days, idx_df)] = sym
    ...
    logger.info(f"Limiting to top {args.top_n_per_day} signals per day sorted by score & adx...")
    signals_df = signals_df.sort_values(by=["signal_date", "score", "adx"], ascending=[True, False, False])
    signals_df = signals_df.groupby("signal_date").head(args.top_n_per_day).reset_index(drop=True)
