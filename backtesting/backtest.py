...
from screener import evaluate, Candidate, _add_indicators
    ...
    for i, (dt, row) in enumerate(future.iterrows(), start=1):
        ...
        if hit_stop:  # conservative: stop wins same-day ties
            ...
            return {"outcome": "stop_loss_hit", "exit_price": exit_price,
                     ...
                     "return_raw_pct": round(raw_ret, 2),
                     "return_pct": round(raw_ret - round_trip, 2)}
        if hit_target:
            exit_price = target
            raw_ret = (exit_price - entry_price) / entry_price * 100
...
def walk_forward_signals(symbol: str, df: pd.DataFrame, backtest_start, backtest_end,
                          min_score: int, max_holding_days: int, index_df: pd.DataFrame = None,
                          custom_weights: dict = None, use_custom_weights: bool = True) -> list:
    """
    ...
    # Pre-calculate indicators once for the entire history to make the walk-forward 100x faster
    try:
        df_indicators = _add_indicators(df)
    except Exception as e:
        logger.warning("Failed to precompute indicators for %s: %s", symbol, e)
        return []
        
    eval_dates = df_indicators.index[(df_indicators.index >= pd.Timestamp(backtest_start)) &
                                     (df_indicators.index <= pd.Timestamp(backtest_end))]

    # Precompute Nifty SMA20
    idx_df = pd.DataFrame()
    if index_df is not None and not index_df.empty:
        idx_df = index_df.copy()
        if isinstance(idx_df.index, pd.DatetimeIndex) and idx_df.index.tz is not None:
            idx_df.index = idx_df.index.tz_localize(None)
        idx_df["sma20"] = ta.trend.sma_indicator(idx_df["Close"], window=20)

    # Load India VIX table
    vix_df = pd.DataFrame()
    try:
        import os
        from config import config
        ...
        if os.path.exists(db_path):
            with duckdb.connect(db_path, read_only=True) as con:
                vix_df = con.execute("SELECT CAST(timezone('Asia/Kolkata', date) AS DATE) AS Date, close AS VIX FROM india_vix").fetchdf()
                if not vix_df.empty:
                    vix_df["Date"] = pd.to_datetime(vix_df["Date"])
                    vix_df = vix_df.set_index("Date")
    except Exception as e:
        logger.warning(f"Could not load historical India VIX for backtest: {e}")

    for signal_date in eval_dates:
        # Check against original df length to ensure proper warmup
        original_slice_len = len(df.loc[:signal_date])
        if original_slice_len < 220:   # need at least 200 warm-up for SMA200
            continue
            
        df_slice = df_indicators.loc[:signal_date]
        
        # Determine base thresholds
        base_t_min = min_score
        base_p_min = max(2, min_score - 1)
        if custom_weights is not None and isinstance(custom_weights, dict):
            if "trend" in custom_weights:
                base_t_min = custom_weights["trend"].get("min_score", min_score)
            elif "min_score" in custom_weights:
                base_t_min = custom_weights["min_score"]
            if "pullback" in custom_weights:
                base_p_min = custom_weights["pullback"].get("min_score", base_p_min)
            elif "min_score" in custom_weights:
                base_p_min = max(2, custom_weights["min_score"] - 1)

        # Apply regime adjustments
        nifty_adj = 0
        vix_adj = 0
        if not idx_df.empty and "sma20" in idx_df.columns:
            idx_slice = idx_df.loc[idx_df.index <= signal_date]
            if not idx_slice.empty:
                last_idx_row = idx_slice.iloc[-1]
                if last_idx_row["Close"] < last_idx_row["sma20"]:
                    nifty_adj = 2
        if not vix_df.empty:
            vix_slice = vix_df.loc[vix_df.index <= signal_date]
            if not vix_slice.empty:
                last_vix = vix_slice.iloc[-1]["VIX"]
                if last_vix > 22.0:
                    vix_adj = 1

        t_min_score = base_t_min + nifty_adj + vix_adj
        p_min_score = base_p_min + nifty_adj + vix_adj

        # Check Trend-continuation class
        try:
            cand_trend = evaluate(symbol, df_slice, skip_fundamental=True, index_df=index_df, 
                                  custom_weights=custom_weights, use_custom_weights=use_custom_weights, 
                                  setup_class="trend")
            if cand_trend.score >= t_min_score:
                future_bars = df.loc[df.index > signal_date]
                if not future_bars.empty:
                    next_open = float(future_bars.iloc[0]["Open"])
                    capped_target = float(cand_trend.target)
                    if next_open < capped_target:
                        outcome = simulate_outcome(
                            df, signal_date=future_bars.index[0] - pd.Timedelta(seconds=1),
                            entry_price=next_open,
                            stop_loss=cand_trend.stop_loss, target=capped_target,
                        ...
                        row = {
                            ...
                            "score": cand_trend.score,
                            "setup_class": "trend",
                            "reasons": "; ".join(cand_trend.reasons),
                            "entry_price": round(next_open, 2),
                            "signal_close": cand_trend.close,
                            "rsi": cand_trend.rsi,
                            "stop_loss": cand_trend.stop_loss,
                            "target": capped_target,
                            **outcome,
                        }
                        for sig_name, sig_val in cand_trend.signals.items():
                            row[f"pointer_{sig_name}"] = sig_val
                        results.append(row)
        except Exception as e:
            logger.debug("Trend eval failed for %s on %s: %s", symbol, signal_date.date(), e)

        # Check Mean-reversion class
        try:
            cand_pb = evaluate(symbol, df_slice, skip_fundamental=True, index_df=index_df, 
                               custom_weights=custom_weights, use_custom_weights=use_custom_weights, 
                               setup_class="pullback")
            if cand_pb.score >= p_min_score:
                future_bars = df.loc[df.index > signal_date]
                if not future_bars.empty:
                    next_open = float(future_bars.iloc[0]["Open"])
                    capped_target = float(cand_pb.target)
                    if next_open < capped_target:
                        outcome = simulate_outcome(
                            df, signal_date=future_bars.index[0] - pd.Timedelta(seconds=1),
                            entry_price=next_open,
                            stop_loss=cand_pb.stop_loss, target=capped_target,
                            max_holding_days=max_holding_days,
                        )
                        row = {
                            "symbol": symbol,
                            ...
                            "score": cand_pb.score,
                            "setup_class": "pullback",
                            "reasons": "; ".join(cand_pb.reasons),
                            "entry_price": round(next_open, 2),
                            "signal_close": cand_pb.close,
                            "rsi": cand_pb.rsi,
                            "stop_loss": cand_pb.stop_loss,
                            "target": capped_target,
                            **outcome,
                        }
                        for sig_name, sig_val in cand_pb.signals.items():
                            row[f"pointer_{sig_name}"] = sig_val
                        results.append(row)
        except Exception as e:
            logger.debug("Pullback eval failed for %s on %s: %s", symbol, signal_date.date(), e)
...
def run_backtest(symbols: list, backtest_months: int = 4, min_score: int = 3,
                  ...
                  progress_callback=None, db_path: str = None,
                  custom_weights: dict = None, use_custom_weights: bool = True,
    ...
    """
    db_path = db_path or config.DUCKDB_PATH
    end = end_date if end_date is not None else get_latest_price_date(db_path, symbols)
    start = start_date if start_date is not None else (end - timedelta(days=int(backtest_months * 31)))
    ...
    def _process(symbol):
        try:
            ...
            sym_weights = None
            if custom_weights:
                sym_weights = custom_weights.get(symbol, None)
            return walk_forward_signals(symbol, df, start, end, min_score, max_holding_days, index_df=index_df, custom_weights=sym_weights, use_custom_weights=use_custom_weights)
        except Exception as e:
...
def compute_accuracy_metrics(results_df: pd.DataFrame) -> dict:
    ...
    # Subclass breakdown (trend vs pullback)
    if "setup_class" in results_df.columns:
        for sc in ["trend", "pullback"]:
            sc_df = results_df[results_df["setup_class"] == sc]
            if not sc_df.empty:
                sc_closed = sc_df[sc_df["outcome"] != "no_data"].copy()
                sc_decided = sc_closed[sc_closed["outcome"].isin(["target_hit", "stop_loss_hit"])]
                sc_n_wins = (sc_decided["outcome"] == "target_hit").sum()
                sc_n_decided = len(sc_decided)
                sc_win_rate = round(100 * sc_n_wins / sc_n_decided, 1) if sc_n_decided else None
                
                sc_closed_for_pct = sc_closed.dropna(subset=["return_pct"])
                sc_avg_return = round(sc_closed_for_pct["return_pct"].mean(), 2) if len(sc_closed_for_pct) else None
                sc_gross_gain = sc_closed_for_pct.loc[sc_closed_for_pct["return_pct"] > 0, "return_pct"].sum()
                sc_gross_loss = -sc_closed_for_pct.loc[sc_closed_for_pct["return_pct"] < 0, "return_pct"].sum()
                sc_pf = round(sc_gross_gain / sc_gross_loss, 2) if sc_gross_loss > 0 else None
                
                metrics[f"class_{sc}"] = {
                    "total_signals": len(sc_df),
                    "decided_signals": sc_n_decided,
                    "target_hits": int(sc_n_wins),
                    "stop_loss_hits": int((sc_decided["outcome"] == "stop_loss_hit").sum()),
                    "win_rate_target_vs_stop_pct": sc_win_rate,
                    "avg_return_pct_per_signal": sc_avg_return,
                    "profit_factor": sc_pf
                }
    return metrics
...
def print_accuracy_report(metrics: dict):
    ...
    # Print class breakdowns if present
    for sc in ["trend", "pullback"]:
        if f"class_{sc}" in metrics:
            cm = metrics[f"class_{sc}"]
            print("-" * 60)
            print(f"Setup Class: {sc.upper()}")
            print(f"  Signals generated            : {cm['total_signals']}")
            print(f"  Win rate (T vs SL)           : {cm['win_rate_target_vs_stop_pct']}%")
            print(f"  Average return               : {cm['avg_return_pct_per_signal']}%")
            print(f"  Profit factor                : {cm['profit_factor']}")
